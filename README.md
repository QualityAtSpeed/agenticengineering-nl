...
- New testimonial → add a `Testimonial` entry in `data/testimonials.ts` with `id` (kebab-case slug), `quoteNL` and `quoteEN` (verbatim quotes in both languages), `name`, and `role`. The `TestimonialCard` component auto-selects the correct quote based on the active locale via `useLocale()`. The section is gated behind `TESTIMONIALS_ENABLED`.
