...
- New route → add it under `app/[locale]/`, give it a `generateMetadata` via `buildPageMetadata({ locale, path, title, description })` (per-page title/description in the `meta.pages.*` keys, both locales), and add its path to `app/sitemap.ts` (the sitemap is an explicit `PATHS` list — training detail pages are derived from `data/trainings.ts`, other routes are listed by hand).
...
lib/
  ...
  page-metadata.ts     # buildPageMetadata({ locale, path, title, description }): Metadata — single source for per-page SEO (canonical, hreflang, OpenGraph)
data/
